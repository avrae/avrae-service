...
class SubscriberMixin(MixinBase, abc.ABC):
    ...
    def is_subscribed(self, user_id: int):
        ...
        return (
            self.sub_coll(current_app.mdb).find_one(
    ...
    def subscribe(self, user_id: int):
        ...
        self.sub_coll(current_app.mdb).insert_one({"type": "subscribe", "subscriber_id": user_id, "object_id": self.id})
...
class EditorMixin(MixinBase, abc.ABC):
    ...
    def is_editor(self, user_id: int):
        ...
        return (
            self.sub_coll(current_app.mdb).find_one({"type": "editor", "subscriber_id": user_id, "object_id": self.id})
        ) is not None
