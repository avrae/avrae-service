...
def guild_permissions_check(user: UserInfo, guild_id: int):
    ...
    raise NeedsServerAliaser(
        "You do not have permissions to edit server collections - either Administrator Discord "
...
def _relevance_based_explore(tags: list, q: str, page: int):
    ...
    es_query = {
        "query": {"bool": {"must": _build_query(tags, q)}},
        "sort": ["_score"],
    ...
    resp = requests.get(f"{config.ELASTICSEARCH_ENDPOINT}/workshop_collections/_search", json=es_query)
...
def _popularity_based_explore(days: int, tags: list, q: str, page: int):
    ...
    es_query = {
        ...
        "aggs": {
            "collections": {
                ...
                "aggs": {"the_score": {"sum": {"field": "sub_score", "missing": 0}}},
            }
        },
    ...
    resp = requests.get(f"{config.ELASTICSEARCH_ENDPOINT}/workshop_events/_search?request_cache=true", json=es_query)
    resp.raise_for_status()
    result = resp.json()
